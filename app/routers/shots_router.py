...
@router.post("/{video_id}", response_model=List[ShotResponse])
def generate_shots(
    video_id: int, 
    db: Session = Depends(get_db), 
    method: DetectionMethod = Query(DetectionMethod.transnet, description="Detection method: transnet or pyscene"),
    threshold: float = Query(0.5, description="Threshold: 0.0-1.0 for TransNetV2, 10.0-50.0 for PySceneDetect")
):
    """
    ...
    if method == DetectionMethod.transnet and not (0.0 <= threshold <= 1.0):
        raise HTTPException(status_code=400, detail="TransNetV2 threshold must be between 0.0 and 1.0")
    elif method == DetectionMethod.pyscene and not (10.0 <= threshold <= 100.0):
        raise HTTPException(status_code=400, detail="PySceneDetect threshold should be between 10.0 and 100.0")
...
def get_shots(video_id: int, db: Session = Depends(get_db)):
    ...
    if not video.shots:
        raise HTTPException(status_code=404, detail="No shots found. Run shot detection first.")
