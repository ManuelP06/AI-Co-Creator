...
def transcribe_video_endpoint(
    video_id: int,
    per_shot: bool = Query(
        True, description="Transcribe per shot (True) or full video (False)"
    ),
        ...
        if per_shot:
            ...
            for i, shot in enumerate(shots):
                ...
                try:
                    ...
                    print(
                        f"Got transcription result: '{transcript_result[:100]}...' (length: {len(transcript_result)})"
                    )
            ...
            db.commit()
            print(
                f"Transcription complete. Processed: {transcribed_count}, Errors: {error_count}"
            )
