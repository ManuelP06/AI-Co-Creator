...
def analyze_video_endpoint(
    video_id: int,
    per_shot: bool = Query(
        True, description="Analysing per shot (True) or full video (False)"
    ),
    ...
    if per_shot:
        try:
            ...
            raise HTTPException(
                status_code=500, detail=f"Could not access shots for video: {str(e)}"
            )
    ...
    try:
        ...
        if per_shot:
            db.refresh(video)
            analyzed_shots = [
                s for s in video.shots if s.analysis and s.analysis.strip()
            ]
...
def clear_video_analyses(
    ...
    try:
        ...
        if clear_all:
            ...
            for shot in shots:
                if shot.analysis and any(
                    pattern in shot.analysis for pattern in error_patterns
                ):
                    shot.analysis = None
                    cleared_count += 1
        ...
        if hasattr(video, "analysis") and video.analysis:
            if clear_all or any(
                pattern in video.analysis for pattern in error_patterns
            ):
                video.analysis = None
                video_cleared = True
        ...
        return {
            ...
            "message": f"Cleared {cleared_count} shot analyses"
            + (" and video analysis" if video_cleared else ""),
        }

    except Exception as e:
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to clear analyses: {str(e)}"
        )
...
def get_analysis_stats(
    ...
    try:
        ...
        with_analysis = sum(
            1 for shot in shots if shot.analysis and shot.analysis.strip()
        )
        empty_analysis = sum(
            1 for shot in shots if not shot.analysis or not shot.analysis.strip()
        )
        ...
        error_analyses = sum(
            ...
            for shot in shots
            if shot.analysis
            and any(pattern in shot.analysis for pattern in error_patterns)
        )
